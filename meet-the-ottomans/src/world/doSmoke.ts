    ...
    public createSmoke(): Entity {
        ...
        const spawnPuff = () => {
            ...
            for (let blobIndex = 0; blobIndex < blobCount; blobIndex += 1) {
                ...
                const horizontalOffset = horiz * (0.2 + (Math.random() * 0.45));
                const heightOffset = ry * (0.05 + (layerT * 1.45));
        ...
        const updateSmoke = (deltaTime: number) => {
            ...
            for (let puffIndex = smokePuffs.length - 1; puffIndex >= 0; puffIndex -= 1) {
                ...
                for (const blob of puff.blobs) {
                    ...
                    const bob = Math.sin((performance.now() * 0.0018) + blob.wobblePhase) * ry * 0.06;
                    const stretch = 1 + (lifeProgress * 0.8);
                    const wobbleX = Math.sin((performance.now() * 0.0012) + blob.wobblePhase) * horiz * 0.05;
                    const wobbleZ = Math.cos((performance.now() * 0.0011) + blob.wobblePhase) * horiz * 0.05;
