...
export class RussianSoldier extends npc {
	...
	constructor(id: number, modelEntity: Entity = new Entity("russian soldier")) {
	...
	private fireProjectileAt(
		...
		const projectile = new Entity("russian bullet");
		projectile.setPosition(origin);
		projectile.lookAt(origin.clone().add(dir));

		const tracer = new Entity("russian bullet tracer");
