...
const groundModelPath = '/world/battlefields/chosin_reservoir.glb';
...
function createStarfieldTexture(device: AppBase['graphicsDevice'], width = 1024, height = 512): Texture {
  ...
  if (!ctx) return new Texture(device!, { mipmaps: true, name: 'chosin-starfield-fallback' });
  ...
  const texture = new Texture(device!, { mipmaps: true, name: 'chosin-starfield' }); texture.setSource(canvas); return texture;
}

export async function battleOfChosinReservoirScene(canvas: HTMLCanvasElement, app: AppBase, _onClick: (battle: Battle) => void, _sceneNum: number, spawnPoint?: [number, number, number]) {
  resetChosinBattleState(); unloadAll(app); app.mouse?.off(); app.keyboard?.off(); if (!canvas) throw new Error('Canvas not found'); const overlay = document.querySelector('.overlay') as HTMLElement | null; const hiddenMap = new Map<HTMLElement, string | null>(); if (overlay) { const children = Array.from(overlay.children) as HTMLElement[]; for (const child of children) { hiddenMap.set(child, child.style.display || null); child.style.display = 'none'; } } const hoverLabel = document.getElementById('battle-hover-label'); if (hoverLabel) hoverLabel.style.display = 'none'; if (!app.graphicsDevice) { const device = await createGraphicsDevice(canvas); const createOptions = new AppOptions(); createOptions.graphicsDevice = device; createOptions.mouse = new Mouse(document.body); createOptions.keyboard = new Keyboard(window); createOptions.touch = new TouchDevice(document.body); createOptions.componentSystems = [RenderComponentSystem, CameraComponentSystem, ScriptComponentSystem, LightComponentSystem, CollisionComponentSystem, RigidBodyComponentSystem]; createOptions.resourceHandlers = [TextureHandler, ContainerHandler]; app.init(createOptions); if (!app.keyboard) app.keyboard = new Keyboard(window); app.setCanvasFillMode(FILLMODE_FILL_WINDOW); app.setCanvasResolution(RESOLUTION_AUTO); const resize = () => app.resizeCanvas(); window.addEventListener('resize', resize); app.once('destroy', () => { window.removeEventListener('resize', resize); for (const [el, prev] of hiddenMap.entries()) { if (prev === null) el.style.removeProperty('display'); else el.style.display = prev; } }); app.start(); }
