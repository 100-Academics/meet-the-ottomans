...
import {
  ...
  DEFAULT_KING_GEORGE_III_BOSS_SPAWN_OPTIONS,
...
export async function battleOfGallipoliScene(
  ...
  addStarField(app, cameraEntity, "gallipoli-star-dome");
  ...
  const npcs = await spawnSceneNpcs(app, rigidbodySystem, GALLIPOLI_NPC_SPAWN_POINTS, npcSpawnOptions);
  ...
  wireBattleInput(app, player, npcs, {
    key2Weapon: 2,
    updateKey: "__gallipoliNpcUpdate",
    initialTotal: GALLIPOLI_NPC_SPAWN_POINTS.length + GALLIPOLI_BOSS_SPAWN_POINT.length,
  });
    ...
    spawnBoss: () => {
      bossState.runBossSpawn(() =>
        spawnSceneBoss({
          ...
          rigidbodySystem,
          npcs,
          spawnPoint: GALLIPOLI_BOSS_SPAWN_POINT,
          bossOptions: DEFAULT_KING_GEORGE_III_BOSS_SPAWN_OPTIONS,
          groundYFallback: respawnGroundY,
        }),
