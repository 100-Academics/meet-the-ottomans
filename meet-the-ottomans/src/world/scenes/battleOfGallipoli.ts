...
function createStarfieldTexture(
  ...
  if (!ctx)
    return new Texture(device!, {
      mipmaps: true,
      name: "gallipoli-starfield-fallback",
    });
...
export async function battleOfGallipoliScene(
  ...
  starMaterial.update();
  const starDome = new Entity("gallipoli-star-dome");
  ...
  bindNpcCombatLoop(app, npcs, () => player.getCameraEntity(), {
    ...
    battleStatus: {
      ...
      initialTotal:
        GALLIPOLI_NPC_SPAWN_POINTS.length + GALLIPOLI_BOSS_SPAWN_POINT.length,
