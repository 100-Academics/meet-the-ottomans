...
function getHighestGroundHitY(
  ...
  const rigidbodySystem = (app.systems as any).rigidbody as
    | {
        ...
        ) => { entity?: Entity | null; point?: Vec3 } | null;
      }
    | undefined;
...
function createStarfieldTexture(
  ...
  if (!ctx) {
    return new Texture(device!, {
      mipmaps: true,
      name: "gettysburg-starfield-fallback",
    });
  }
...
export async function battleOfGettysburgScene(
  ...
  bindNpcCombatLoop(app, npcs, () => player.getCameraEntity(), {
    ...
    battleStatus: {
      ...
      initialTotal:
        GETTYSBURG_NPC_SPAWN_POINTS.length + GETTYSBURG_BOSS_SPAWN_POINT.length,
