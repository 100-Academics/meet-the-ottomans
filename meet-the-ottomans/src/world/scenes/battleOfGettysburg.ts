...
function createStarfieldTexture(device: AppBase['graphicsDevice'], width = 1024, height = 512): Texture {
  ...
  if (!ctx) {
    return new Texture(device!, { mipmaps: true, name: 'gettysburg-starfield-fallback' });
  }
  ...
  const texture = new Texture(device!, { mipmaps: true, name: 'gettysburg-starfield' });
...
export async function battleOfGettysburgScene(
  ...
  await new Promise<void>((resolve) => {
    if (envAtlasAsset.loaded) { resolve(); return; }
  ...
  const starMesh = createSphere(app.graphicsDevice, { radius: 220, latitudeBands: 64, longitudeBands: 64 });
  starDome.addComponent('render', { meshInstances: [new MeshInstance(starMesh, starMaterial)] });
  ...
  if (app.systems.light) {
    ...
    light.addComponent('light', { type: 'directional', color: new Color(1, 1, 1), intensity: 1, castShadows: true });
  ...
  app.keyboard?.on('keydown', (event: { key: number | null }) => {
    ...
    if (event.key === KEY_1) { player.equipWeapon(1); updateBattleHUD(player); }
    else if (event.key === KEY_2) { player.equipWeapon(3); updateBattleHUD(player); }
  });
  ...
  app.mouse?.on('mousedown', (event: { x: number; y: number; button: number }) => {
    ...
    if (hitNpc) {
      console.log(`Hit NPC`);
      try { if ((hitNpc as any) instanceof Boss) { (hitNpc as unknown as Boss).updateHealthBar(); } } catch (e) {}
    }
  });
  ...
  bindNpcCombatLoop(app, npcs, () => player.getCameraEntity(), {
    ...
    onNpcAttack: (attacker, target, damage) => {
      ...
      try { if ((target as any) instanceof Boss) { (target as unknown as Boss).updateHealthBar(); } } catch (e) {}
