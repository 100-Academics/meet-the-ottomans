...
export async function battleOfKyivScene(
  ...
  addStarField(app, cameraEntity, "kyiv-star-dome");
  ...
  const npcs = await spawnSceneNpcs(app, rigidbodySystem, KYIV_NPC_SPAWN_POINTS, npcSpawnOptions);
  ...
  wireBattleInput(app, player, npcs, {
    key2Weapon: 2,
    updateKey: "__kyivNpcUpdate",
    initialTotal: KYIV_NPC_SPAWN_POINTS.length + KYIV_BOSS_SPAWN_POINT.length,
  });
    ...
    spawnBoss: () => {
      bossState.runBossSpawn(() =>
        spawnSceneBoss({
          ...
          rigidbodySystem,
          npcs,
          spawnPoint: KYIV_BOSS_SPAWN_POINT,
          bossOptions: DEFAULT_STALIN_BOSS_SPAWN_OPTIONS,
          groundYFallback: respawnGroundY,
        }),
