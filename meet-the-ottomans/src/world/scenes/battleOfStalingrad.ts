...
function resetStalingradBattleState(): void {
  isBossSpawned = false;
...
function createStarfieldTexture(device: AppBase['graphicsDevice'], width = 1024, height = 512): Texture {
  ...
  if (!ctx) return new Texture(device!, { mipmaps: true, name: 'stalingrad-starfield-fallback' });
  const baseGradient = ctx.createLinearGradient(0, 0, width, height); baseGradient.addColorStop(0, '#000000'); baseGradient.addColorStop(0.5, '#04040b'); baseGradient.addColorStop(1, '#000000'); ctx.fillStyle = baseGradient; ctx.fillRect(0,0,width,height);
  const nebulae = [ { x: width * 0.2, y: height * 0.3, r: width * 0.18, color: 'rgba(70, 120, 255, 0.16)' }, { x: width * 0.7, y: height * 0.22, r: width * 0.14, color: 'rgba(160, 110, 255, 0.12)' }, { x: width * 0.75, y: height * 0.7, r: width * 0.22, color: 'rgba(60, 190, 255, 0.14)' } ]; nebulae.forEach((nebula) => { const glow = ctx.createRadialGradient(nebula.x, nebula.y, 0, nebula.x, nebula.y, nebula.r); glow.addColorStop(0, nebula.color); glow.addColorStop(1, 'rgba(0, 0, 0, 0)'); ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(nebula.x, nebula.y, nebula.r, 0, Math.PI * 2); ctx.fill(); });
  ...
  const texture = new Texture(device!, { mipmaps: true, name: 'stalingrad-starfield' }); texture.setSource(canvas); return texture;
}

export async function battleOfStalingradScene(canvas: HTMLCanvasElement, app: AppBase, _onClick: (battle: Battle) => void, _sceneNum: number, spawnPoint?: [number, number, number]) {
  resetStalingradBattleState(); unloadAll(app); app.mouse?.off(); app.keyboard?.off(); if (!canvas) throw new Error('Canvas not found'); const overlay = document.querySelector('.overlay') as HTMLElement | null; const hiddenMap = new Map<HTMLElement, string | null>(); if (overlay) { const children = Array.from(overlay.children) as HTMLElement[]; for (const child of children) { hiddenMap.set(child, child.style.display || null); child.style.display = 'none'; } } const hoverLabel = document.getElementById('battle-hover-label'); if (hoverLabel) hoverLabel.style.display = 'none'; if (!app.graphicsDevice) { const device = await createGraphicsDevice(canvas); const createOptions = new AppOptions(); createOptions.graphicsDevice = device; createOptions.mouse = new Mouse(document.body); createOptions.keyboard = new Keyboard(window); createOptions.touch = new TouchDevice(document.body); createOptions.componentSystems = [RenderComponentSystem, CameraComponentSystem, ScriptComponentSystem, LightComponentSystem, CollisionComponentSystem, RigidBodyComponentSystem]; createOptions.resourceHandlers = [TextureHandler, ContainerHandler]; app.init(createOptions); if (!app.keyboard) app.keyboard = new Keyboard(window); app.setCanvasFillMode(FILLMODE_FILL_WINDOW); app.setCanvasResolution(RESOLUTION_AUTO); const resize = () => app.resizeCanvas(); window.addEventListener('resize', resize); app.once('destroy', () => { window.removeEventListener('resize', resize); for (const [el, prev] of hiddenMap.entries()) { if (prev === null) el.style.removeProperty('display'); else el.style.display = prev; } }); app.start(); }
