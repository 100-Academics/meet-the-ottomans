...
function createStarfieldTexture(
  ...
  if (!ctx)
    return new Texture(device!, {
      mipmaps: true,
      name: "stalingrad-starfield-fallback",
    });
...
export async function battleOfStalingradScene(
  ...
  starMaterial.update();
  const starDome = new Entity("stalingrad-star-dome");
  ...
  bindNpcCombatLoop(app, npcs, () => player.getCameraEntity(), {
    ...
    battleStatus: {
      ...
      initialTotal:
        STALINGRAD_NPC_SPAWN_POINTS.length + STALINGRAD_BOSS_SPAWN_POINT.length,
