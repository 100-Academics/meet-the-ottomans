...
function resetThreeEmperorsBattleState(): void {
  bossState.reset();
  Mongol.resetBattleState();
}

export async function battleOfThreeEmperorsScene(
  ...
  addStarField(app, cameraEntity, "austerlitz-star-dome");
  ...
  const npcs = await spawnSceneNpcs(app, rigidbodySystem, THREE_EMPERORS_NPC_SPAWN_POINTS, npcSpawnOptions);
  ...
  wireBattleInput(app, player, npcs, {
    key2Weapon: 2,
    updateKey: "__threeEmperorsNpcUpdate",
    initialTotal: THREE_EMPERORS_NPC_SPAWN_POINTS.length + THREE_EMPERORS_BOSS_SPAWN_POINT.length,
  });
  ...
  bindVictoryCheck(app, {
    ...
    onVictory: () => {
      removeBattleHUD();
      triggerVictory('Battle of Three Emperors', canvas, app);
    },
    spawnBoss: () => {
      bossState.runBossSpawn(() =>
        spawnSceneBoss({
          ...
          rigidbodySystem,
          npcs,
          spawnPoint: THREE_EMPERORS_BOSS_SPAWN_POINT,
          bossOptions: DEFAULT_NAPOLEON_BOSS_SPAWN_OPTIONS,
          groundYFallback: respawnGroundY,
        }),
