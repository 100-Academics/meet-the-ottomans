...
function createStarfieldTexture(
  ...
  if (!ctx)
    return new Texture(device!, {
      mipmaps: true,
      name: "austerlitz-starfield-fallback",
    });
...
export async function battleOfThreeEmperorsScene(
  ...
  starMaterial.update();
  const starDome = new Entity("austerlitz-star-dome");
  ...
  bindNpcCombatLoop(app, npcs, () => player.getCameraEntity(), {
    ...
    battleStatus: {
      ...
      initialTotal:
        THREE_EMPERORS_NPC_SPAWN_POINTS.length +
        THREE_EMPERORS_BOSS_SPAWN_POINT.length,
