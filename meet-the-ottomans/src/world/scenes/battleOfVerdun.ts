...
export async function battleOfVerdunScene(
  ...
  addStarField(app, cameraEntity, "verdun-star-dome");

  const groundResult = await loadBattleGround(app, groundModelPath, player,
    { awaitAmmo: true, centerSearch: true, movementBounds: true, centerFail: "ammoHit" });
  ...
  const npcs = await spawnSceneNpcs(app, rigidbodySystem, VERDUN_NPC_SPAWN_POINTS, npcSpawnOptions);
  ...
  wireBattleInput(app, player, npcs, {
    ...
    initialTotal: VERDUN_NPC_SPAWN_POINTS.length + VERDUN_BOSS_SPAWN_POINT.length,
  });
    ...
    spawnBoss: () => {
      bossState.runBossSpawn(() =>
        spawnSceneBoss({
          ...
          rigidbodySystem,
          npcs,
          spawnPoint: VERDUN_BOSS_SPAWN_POINT,
