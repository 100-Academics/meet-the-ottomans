...
function createStarfieldTexture(
  ...
  if (!ctx)
    return new Texture(device!, {
      mipmaps: true,
      name: "saigon-starfield-fallback",
    });
...
export async function fallOfSaigonScene(
  ...
  starMaterial.update();
  const starDome = new Entity("saigon-star-dome");
  ...
  bindNpcCombatLoop(app, npcs, () => player.getCameraEntity(), {
    ...
    battleStatus: {
      ...
      initialTotal:
        SAIGON_NPC_SPAWN_POINTS.length + SAIGON_BOSS_SPAWN_POINT.length,
