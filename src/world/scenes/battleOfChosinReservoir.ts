...
export async function battleOfChosinReservoirScene(
  ...
  addStarField(app, cameraEntity, "chosin-star-dome");

  const groundResult = await loadBattleGround(app, groundModelPath, player,
    { awaitAmmo: true, centerSearch: true, movementBounds: true, useTerrainBounds: true });
  ...
  const npcs = await spawnSceneNpcs(app, rigidbodySystem, CHOSIN_RESERVOIR_NPC_SPAWN_POINTS, npcSpawnOptions);
  ...
  wireBattleInput(app, player, npcs, {
    ...
    initialTotal: CHOSIN_RESERVOIR_NPC_SPAWN_POINTS.length + CHOSIN_RESERVOIR_BOSS_SPAWN_POINT.length,
  });
    ...
    spawnBoss: () => {
      bossState.runBossSpawn(() =>
        spawnSceneBoss({
          ...
          rigidbodySystem,
          npcs,
          spawnPoint: CHOSIN_RESERVOIR_BOSS_SPAWN_POINT,
          bossOptions: DEFAULT_NINE_TAILED_FOX_BOSS_SPAWN_OPTIONS,
          groundYFallback: respawnGroundY,
        }),
