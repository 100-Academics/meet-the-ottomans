...
import {
  ...
  DEFAULT_LENIN_BOSS_SPAWN_OPTIONS,
  STALINGRAD_BOSS_SPAWN_POINT,
  STALINGRAD_NPC_SPAWN_POINTS,
...
export async function battleOfStalingradScene(
  ...
  addStarField(app, cameraEntity, "stalingrad-star-dome");
  ...
  const npcs = await spawnSceneNpcs(app, rigidbodySystem, STALINGRAD_NPC_SPAWN_POINTS, npcSpawnOptions);
  ...
  wireBattleInput(app, player, npcs, {
    key2Weapon: 2,
    updateKey: "__stalingradNpcUpdate",
    initialTotal: STALINGRAD_NPC_SPAWN_POINTS.length + STALINGRAD_BOSS_SPAWN_POINT.length,
  });
  ...
  bindVictoryCheck(app, {
    ...
    onVictory: () => {
      removeBattleHUD();
      triggerVictory('Battle of Stalingrad', canvas, app);
    },
    spawnBoss: () => {
      bossState.runBossSpawn(() =>
        spawnSceneBoss({
          ...
          rigidbodySystem,
          npcs,
          spawnPoint: STALINGRAD_BOSS_SPAWN_POINT,
          bossOptions: DEFAULT_LENIN_BOSS_SPAWN_OPTIONS,
          groundYFallback: respawnGroundY,
        }),
